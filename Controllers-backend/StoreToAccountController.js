...
// function create store_to_account
async function create_store_to_account(req, res) {
    ...
    try {
        await db.StoreToAccount.create({
...
//get all store_to_account
async function get_all_store_to_account(req, res) {
    try {
...
//get store_to_account by id
async function get_store_to_account_by_id(req, res) {
    try {
        let data = await db.StoreToAccount.findByPk(req.params.id);
...
//update store_to_account
async function update_store_to_account(req, res) {
    ...
    try {
        let row = await db.StoreToAccount.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.StoreToAccount.update(req.body, { where: { id: req.params.id } });
...
//update store_to_account isActive
async function update_store_to_account_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.StoreToAccount.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    //exprot function
    create_store_to_account,
    get_all_store_to_account,
    get_store_to_account_by_id,
    update_store_to_account,
    update_store_to_account_isActive,
