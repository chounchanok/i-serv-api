...
// function create Competitor
async function create_Competitor(req, res) {
...
//get all Competitor
async function get_all_Competitor(req, res) {
    try {
        ...
        let data = await db.Competitor.findAll({
...
async function get_all_Competitor_filter(req, res) {
    try {
        const whereConditions = {};
        if (req.body.group_customer_id) whereConditions.group_customer_id = req.body.group_customer_id;
        let data = await db.Competitor.findAll({
...
async function get_Competitor_by_id(req, res) {
    try {
        let data = await db.Competitor.findByPk(req.params.id);
...
//update Competitor
async function update_Competitor(req, res) {
    ...
    try {
        let row = await db.Competitor.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Competitor.update(req.body, { where: { id: req.params.id } });
...
//update Competitor isActive
async function update_Competitor_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Competitor.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    create_Competitor,
    get_all_Competitor,
    get_all_Competitor_filter,
    get_Competitor_by_id,
    update_Competitor,
    update_Competitor_isActive,
