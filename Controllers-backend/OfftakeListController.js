...
async function create_OfftakeList(req, res) {
    const error = validation(req);
    if (error) {
        return res.status(422).json(error);
    }
    try {
        await db.OfftakeList.create(req.body)
...
async function createOrUpdate_Offtake(req, res) {
    const error = validation(req);
    if (error) {
        return res.status(422).json(error);
    }

    const OfftakeList = req.body.group_id; // สมมุติว่า array ถูกส่งเข้ามาใน req.body

    try {
        for (const item of OfftakeList) {
            if (item.id === null) {
...
async function createOrUpdate_OfftakeList(req, res) {
    const error = validation(req);
    if (error) {
        return res.status(422).json(error);
    }

    const OfftakeList = req.body; // สมมุติว่า array ถูกส่งเข้ามาใน req.body
    ...
    try {
        ...
        for (const item of OfftakeList) {
            if (item.id) {
                await db.OfftakeList.update({
                    amount: item.amount,
                    offtake_status: item.offtake_status,
                ...
                const OfftakeList = await db.OfftakeList.findOne({
                    where: { 
                        id: item.id
                    }
                });
                offtake_idzzz = OfftakeList.offtake_id
            }
        }
        ...
        if (offtake_idzzz > 0) {
            ...
            SELECT 
                ...
                WHERE tb_map_user_store_list.user_id = '${Offtake.user_id}' AND tb_map_user_store_list.store_id != '${Offtake.store_id}'
                AND store.group_customer_id = '${Offtake.store.group_customer_id}'
                AND store.account_id = '${Offtake.store.account_id}'
                AND store.account_type_id = '${Offtake.store.account_type_id}'
            ...
            if(rawData.length>0){
                for (const product of rawData) {
                    const count = await db.Offtake.count({
                        where: {
                            group_id: Offtake.group_id,
                            store_id: product.store_id,
                            datenow: Offtake.datenow,
                            datesave_start: Offtake.datesave_start,
                            datesave_end: Offtake.datesave_end,
                            user_id: Offtake.user_id,
                        }
                    });
                    if (count === 0) {
                            ...
                            if(productList){
                                for (const product of productList) {
                                    await db.OfftakeList.create({
                                        ...
                                        // offtake_status: product.offtake_status,
                                        // not_sell: product.not_sell,
                                        // note: product.note,
                                        isActive: 'Y'
                                    });
                                }
...
//get all OfftakeList
async function get_all_OfftakeList(req, res) {
    try {
        let data = await db.OfftakeList.findAll({
            include: [
                {
                    model: db.Offtake,
                    as: 'offtake', // ชื่อ alias ที่ตั้งไว้ใน model
                    required: false, // เลือกเฉพาะฟิลด์ที่ต้องการ (เช่น name)
                },
                {
                    model: db.Product,
...
async function get_all_OfftakeList_filter(req, res) {
    try {
...
//get OfftakeList by id
async function get_OfftakeList_by_id(req, res) {
    try {
        let data = await db.OfftakeList.findByPk(req.params.id);
...
//update OfftakeList
async function update_OfftakeList(req, res) {
    ...
    try {
        let row = await db.OfftakeList.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.OfftakeList.update(req.body, { where: { id: req.params.id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
...
//update OfftakeList isActive
async function update_OfftakeList_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.OfftakeList.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    //exprot function
    create_OfftakeList,
    createOrUpdate_OfftakeList,
    createOrUpdate_Offtake,
    get_all_OfftakeList,
    get_all_OfftakeList_filter,
    get_OfftakeList_by_id,
    update_OfftakeList,
    update_OfftakeList_isActive,
