...
async function create_promotion(req, res) {
...
//get all promotion
async function get_all_promotion(req, res) {
    try {
...
//get promotion by id
async function get_promotion_by_id(req, res) {
    try {
        let data = await db.Promotion.findByPk(req.params.id);
...
//update promotion
async function update_promotion(req, res) {
    ...
    try {
        let row = await db.Promotion.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Promotion.update(req.body, { where: { id: req.params.id } });
...
//update promotion isActive
async function update_promotion_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Promotion.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
