...
//create sub category
async function create_sub_category(req, res) {
...
async function get_all_sub_category(req, res) {
    try {
...
async function get_sub_category_by_id(req, res) {
    try {
        let data = await db.SubCategory.findByPk(req.params.id);
...
async function update_sub_category(req, res) {
    ...
    try {
        let row = await db.SubCategory.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.SubCategory.update(req.body, { where: { id: req.params.id } });
...
async function is_active_sub_category(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.SubCategory.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
