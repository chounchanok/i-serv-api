...
async function import_posm(req, res) {
    ...
    try {
        ...
        let insert_data = await insert_posm(data);
        return res.send({ status: "success", data: data });

    } catch (error) {
        console.error("Error in import_posm: ", error);
...
async function read_excel(path) {
    try {
        ...
        throw new Error("Error reading Excel file"); // Throw an error to be handled in import_posm
    }
}


//insert posm
async function insert_posm(data) {
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            const posm = await db.Posm.findOne({
                where: whereClause
            });
            if (!posm) {
            //     // Create posm entry
        ...
        //console.log('posm inserted successfully');
    } catch (error) {
        console.error("Error inserting data: ", error.message);
        throw new Error("Error inserting data"); // Throw an error to be handled in import_posm
    }
}
module.exports = {
    import_posm,
