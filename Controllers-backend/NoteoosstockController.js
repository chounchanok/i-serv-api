...
// function create Noteoosstock
async function create_Noteoosstock(req, res) {
...
//get all Noteoosstock
async function get_all_Noteoosstock(req, res) {
    try {
        ...
        let data = await db.Noteoosstock.findAll({
...
async function get_all_Noteoosstock_filter(req, res) {
    try {
        const whereConditions = {};
        if (req.body.group_id) {
            whereConditions.group_customer_id = req.body.group_id;
        }
...
//get Noteoosstock by id
async function get_Noteoosstock_by_id(req, res) {
    try {
        let data = await db.Noteoosstock.findByPk(req.params.id);
...
//update Noteoosstock
async function update_Noteoosstock(req, res) {
    ...
    try {
        let row = await db.Noteoosstock.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Noteoosstock.update(req.body, { where: { id: req.params.id } });
...
//update Noteoosstock isActive
async function update_Noteoosstock_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Noteoosstock.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    //exprot function
    create_Noteoosstock,
    get_all_Noteoosstock,
    get_Noteoosstock_by_id,
    update_Noteoosstock,
    update_Noteoosstock_isActive,
    get_all_Noteoosstock_filter,
