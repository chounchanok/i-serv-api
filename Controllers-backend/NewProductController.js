...
// function create new_product
async function create_new_product(req, res) {
...
//get all new_product
async function get_all_new_product(req, res) {
    try {
        let data = await db.NewProduct.findAll({
            include: [
                {
                    model: db.Brand,
...
//get new_product by id
async function get_new_product_by_id(req, res) {
    try {
        let data = await db.NewProduct.findByPk(req.params.id);
...
//update new_product
async function update_new_product(req, res) {
    ...
    try {
        let row = await db.NewProduct.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.NewProduct.update(req.body, { where: { id: req.params.id } });
...
//update new_product isActive
async function update_new_product_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.NewProduct.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    create_new_product,
    get_all_new_product,
    get_new_product_by_id,
    update_new_product,
    update_new_product_isActive,
