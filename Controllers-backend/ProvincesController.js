...
// function create provinces
async function create_provinces(req, res) {
    ...
    try {
        await db.Provinces.create({
            name_in_thai: req.body.name_in_thai,
            name_in_english: req.body.name_in_english,
...
//get all provinces
async function get_all_provinces(req, res) {
    try {
        let data = await db.Provinces.findAll();
...
//get provinces by id
async function get_provinces_by_id(req, res) {
    try {
        let data = await db.Provinces.findByPk(req.params.id);
...
//update provinces
async function update_provinces(req, res) {
    ...
    try {
        let row = await db.Provinces.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Provinces.update(req.body, { where: { id: req.params.id } });
...
//update provinces isActive
async function update_provinces_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Provinces.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
