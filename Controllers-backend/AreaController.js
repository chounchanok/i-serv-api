...
async function create_area(req, res) {
...
//get all area
async function get_all_area(req, res) {
    try {
...
//get area by id
async function get_area_by_id(req, res) {
    try {
        let data = await db.Area.findByPk(req.params.id);
...
async function update_area(req, res) {
    ...
    try {
        let row = await db.Area.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Area.update(req.body, { where: { id: req.params.id } });
...
//update area isActive
async function update_area_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Area.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
