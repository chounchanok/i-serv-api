...
// function create area_supervisor
async function create_area_supervisor(req, res) {
    ...
    try {
        await db.AreaSupervisor.create({
            name: req.body.name,
...
//get all area_supervisor
async function get_all_area_supervisor(req, res) {
    try {
        // let data = await db.AreaSupervisor.findAll();
...
async function get_all_area_supervisor_list(req, res) {
    try {
        // let data = await db.AreaSupervisor.findAll();
        const whereConditions = {};
        if (req.body.position_name != "SuperAdmin") {
...
async function filter_all_area_supervisor(req, res) {
    try {
        // let data = await db.AreaSupervisor.findAll();
        const whereConditions = {};
        if (req.body.group_customer_id) whereConditions.group_customer_id = req.body.group_customer_id;
...
//get area_supervisor by id
async function get_area_supervisor_by_id(req, res) {
    try {
        let data = await db.AreaSupervisor.findByPk(req.params.id);
...
async function update_area_supervisor(req, res) {
    ...
    try {
        let row = await db.AreaSupervisor.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.AreaSupervisor.update(req.body, { where: { id: req.params.id } });
...
//update area_supervisor isActive
async function update_area_supervisor_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.AreaSupervisor.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    //exprot function
    create_area_supervisor,
    get_all_area_supervisor,
    get_all_area_supervisor_list,
    filter_all_area_supervisor,
    get_area_supervisor_by_id,
    update_area_supervisor,
    update_area_supervisor_isActive,
