...
async function create_reason(req, res) {
...
//get all reason
async function get_all_reason(req, res) {
    try {
        ...
        let data = await db.ReasonForNotGettingSpace.findAll({
...
async function get_all_reason_filter(req, res) {
    try {
        const whereConditions = {};
        if (req.body.group_customer_id) whereConditions.group_customer_id = req.body.group_customer_id;
        let data = await db.ReasonForNotGettingSpace.findAll({
...
//get reason by id
async function get_reason_by_id(req, res) {
    try {
        let data = await db.ReasonForNotGettingSpace.findByPk(req.params.id);
...
//update reason
async function update_reason(req, res) {
    ...
    try {
        let row = await db.ReasonForNotGettingSpace.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.ReasonForNotGettingSpace.update(req.body, { where: { id: req.params.id } });
...
//update reason isActive
async function update_reason_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.ReasonForNotGettingSpace.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
...
module.exports = {
    ...
    create_reason,
    get_all_reason,
    get_all_reason_filter,
    get_reason_by_id,
    update_reason,
    update_reason_isActive,
