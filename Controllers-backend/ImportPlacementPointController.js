...
async function import_PlacementPoint(req, res) {
    ...
    try {
        ...
        let insert_data = await insert_PlacementPoint(data);
        return res.send({ status: "success", data: data });

    } catch (error) {
        console.error("Error in import_PlacementPoint: ", error);
...
async function read_excel(path) {
    try {
        ...
        throw new Error("Error reading Excel file"); // Throw an error to be handled in import_PlacementPoint
    }
}


//insert PlacementPoint
async function insert_PlacementPoint(data) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const PlacementPoint = await db.PlacementPoint.findOne({
                where: whereClause
            });
            if (!PlacementPoint) {
            //     // Create PlacementPoint entry
        ...
        //console.log('PlacementPoint inserted successfully');
    } catch (error) {
        console.error("Error inserting data: ", error.message);
        throw new Error("Error inserting data"); // Throw an error to be handled in import_PlacementPoint
    }
}
module.exports = {
    import_PlacementPoint,
}
