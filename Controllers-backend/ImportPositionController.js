...
async function import_position(req, res) {
    ...
    try {
        ...
        let insert_data = await insert_position(data);
        return res.send({ status: "success", data: data });

    } catch (error) {
        console.error("Error in import_position: ", error);
...
async function read_excel(path) {
    try {
        ...
        throw new Error("Error reading Excel file"); // Throw an error to be handled in import_position
    }
}


//insert position
async function insert_position(data) {
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            let group_customer_id_new = 0;
            if (!group_customer_id) {
                if(data[i].group_customer_id){
                    if(data[i].group_customer_id == ""){
                        group_customer_id_new = 0;
                    }else{
            ...
            const whereClause = {
                ...
                permission_name: data[i].permission_name,
                name: data[i].name
            };
            ...
            //     // Create position entry
                await db.Position.create({
                    group_customer_id: group_customer_id_new,
                    permission_name: data[i].permission_name != undefined ? data[i].permission_name : null,
        ...
        throw new Error("Error inserting data"); // Throw an error to be handled in import_position
    }
}
module.exports = {
    import_position,
