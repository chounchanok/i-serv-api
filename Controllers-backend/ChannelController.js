...
async function create_channel(req, res) {
...
//get all channel
async function get_all_channel(req, res) {
    try {
        ...
        let data = await db.Channel.findAll({
            where: whereConditions,
            include: [
                {
...
//get channel by id
async function get_channel_by_id(req, res) {
    try {
        let data = await db.Channel.findByPk(req.params.id);
...
//update channel
async function update_channel(req, res) {
    ...
    try {
        let row = await db.Channel.findByPk(req.params.id);
        if (!row) {
            throw new Error('ไม่สามารถทำรายการได้ เนื่องจากไม่พบรายการที่ต้องการแก้ไข');
        }
        await db.Channel.update(req.body, { where: { id: req.params.id } });
...
//update channel isActive
async function update_channel_isActive(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        await db.Channel.update(req.body, { where: { id: id } });
        res.send({ status: "success", message: "บันทึกข้อมูลเรียบร้อย" });
    } catch (err) {
        res.status(500).send({ status: "error", message: err.message || "ไม่สามารถบันทึกข้อมูลที่เลือกได้!" });
    }
