...
async function import_Noteoosstock(req, res) {
    ...
    try {
        ...
        let insert_data = await insert_Noteoosstock(data);
        return res.send({ status: "success", data: data });

    } catch (error) {
        console.error("Error in import_Noteoosstock: ", error);
...
async function read_excel(path) {
    try {
        ...
        throw new Error("Error reading Excel file"); // Throw an error to be handled in import_Noteoosstock
    }
}


//insert Noteoosstock
async function insert_Noteoosstock(data) {
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            const Noteoosstock = await db.Noteoosstock.findOne({
                where: whereClause
            });
            if (!Noteoosstock) {
            //     // Create Noteoosstock entry
        ...
        //console.log('Noteoosstock inserted successfully');
    } catch (error) {
        console.error("Error inserting data: ", error.message);
        throw new Error("Error inserting data"); // Throw an error to be handled in import_Noteoosstock
    }
}
module.exports = {
    import_Noteoosstock,
